...
Init <- function(sim) {
  ...
  studyArea.buff <- st_buffer(sfcoords, dist = 100000)
...
.inputObjects <- function(sim) {
  ...
  if (!suppliedElsewhere("boo", sim = sim)) {
    ...
    if ("ON" %in% Par$jurisdiction == TRUE){
...
dataPrep_BC <- function(dPath=dPath, bc_layers, layers) {
  ...
  # round 6 digits lat/long for ease across OS
  dat[,`:=`(Latitude = round(Latitude, digits = 6), Longitude = round(Longitude, digits = 6))]
  #check for complete entries and remove any incomplete data
